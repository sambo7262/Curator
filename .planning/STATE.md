...
**Last action**: Pushed Phase 2 to main and began on-NAS deploy/smoke. Confirmed: cold-start /healthz live (phase 2); Lidarr READ path works (detector fetched real gaps). Surfaced + fixed a deploy issue: the standalone `python -m core.gap_detector` (separate process) hit `database is locked` against the live uvicorn app's retained single-writer connection (accepted-risk T-02-06 made concrete). Fix: added **`POST /detect`** to main.py (commit 1404228, pushed) — runs detect_gaps on `app.state.db` (the ONE writer), non-blocking lock → 409 on concurrent calls; this is also how Phase 5's scheduler will trigger. Tests: end-to-end write+dedup + 409 guard; suite **33 passed**. Deployed compose drift noted: owner's running YAML uses LAN-IP *arr URLs (192.168.86.37) + had to add the Phase-2 `/db` mount, DB_PATH, and `user:` line (the missing `/db` mount was the cold-start crash).
**Next action**: CI building image w/ /detect (commit 1404228). When green → redeploy curator on NAS → trigger detection via `curl -X POST http://192.168.86.37:8674/detect` (NOT the docker-exec CLI — that collides with the live app). Expect `{"status":"ok","detected":{"lidarr":N,...}}`; re-POST shouldn't grow row count (dedup). That closes UAT test 6. Then `/clear` → `/gsd:plan-phase 3` (Matching & Quality Gating; RESEARCH-SEED.md has beets-distance-model + 1σ-threshold queued). Still TODO (offered, not yet done): reconcile repo docker-compose.yml to the owner's LAN-IP reality + add the `/db` dir step to DEPLOY.md.
