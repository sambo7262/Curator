...
- **D-11:** Before the **first live download test**, basic slskd sharing must be configured **manually in slskd.yml** (share `/data/media/music` + `/data/media/books`, the clean library — NOT the download/staging tree) and verified `shared file count > 0`. This is slskd *config* (Phase 1 infra concern), distinct from the Curator-automated share self-healing of SHARE-01/02 (Phase 5). Phase 4 **code does not configure shares** — the plan includes a verification step/precondition note that gates the live test; unit/integration tests against fakes do not need it. **STATUS: shares confirmed live by owner 2026-05-31.**
- **D-12 (path-identity / download-dir precondition — owner setup, NOT Curator code):** Files move as **slskd downloads into a staging dir on `/data` → Curator calls the *arr Manual Import API → the *arr hardlink-Moves the wanted files into its library root folder** (Curator never moves bytes). For the Move to be an atomic hardlink (the #1 import-failure cause), four owner-side setup items must hold, and they are SEPARATE from the D-11 shares: (1) **slskd `directories.downloads` → `/data/downloads/soulseek`** (slskd.yml — distinct from `shares.directories`; default `/app/downloads` is the WRONG tree) and it must MATCH Curator's `staging_root`; (2) pre-create `/volume1/data/downloads/soulseek` owned `1031:65536`; (3) **curator `/data` mount `:ro` → read-write** (compose edit); (4) verify the *arr mounts the FULL `/volume1/data:/data` and its root folder is `/data/media/music` (resp. books) so it can SEE the staging dir for the hardlink Move. **Full owner checklist with commands lives in `DEPLOY.md` §"Step 8 — Phase 4 setup".** Phase 4 code assumes/verifies these; plan 04-05's live-test gate references them.
